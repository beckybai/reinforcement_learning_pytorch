...
class REINFORCEAgent():
	def __init__(self, state_dim, action_dim, learning_rate=0.0001, reward_decay=0.99, e_greedy=0.9):
		...
		self.model = QNet_policy(self.state_dim, self.action_dim).cuda() if use_cuda else QNet_policy(self.state_dim,
		                                                                                              self.action_dim)
	...
	def select_action(self, state):
		if (self.greedy):
			...
			return np.random.choice(self.action_dim, 1, p=p_actions)[
				0]  # the return of random.choice is a list, index zero aims to pick a number :)
