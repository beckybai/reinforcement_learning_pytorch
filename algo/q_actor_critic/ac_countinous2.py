...
# import datetime
from algo.q_actor_critic.ACAgent_con_td_offline import ACAgent_con
# import random
...
def run_episode(env, qf):  # on line algorithm
	...
	while (not done) and t < MAX_ITER:
		...
		# env.render()
		obs = obs_new
		state = np.float64(obs)
		action = qf.select_action(state)
		action_data = action.data.tolist()[0]
		obs_new, reward, done, _ = env.step(action_data)
		reward = reward
		add_reward += reward
		# qf.trajectory.append([np.float32(obs),np.float32(action_data),np.float32(obs_new),reward])
		
		# qf.update(obs,action_data,obs_new,reward) # On-Line
		if (done):
			new_state = np.float64(0 * obs_new)
			state = np.float64(0 * obs)
		else:
			state = np.float64(obs)
			new_state = np.float64(obs_new)
		
		qf.buffer.append([state, np.float64(action_data), new_state, reward])
